 ...
 * Optionally bias by user coordinates (lat/lng)
 */
async function fetchAllPlaces(searchQuery, apiKey, maxPages = 2, userCoords = null) {
  let allResults = []
  ...
  do {
    ...
    // Bias results towards the user's actual location if provided
    if (userCoords?.lat && userCoords?.lng) {
      url += `&location=${userCoords.lat},${userCoords.lng}&radius=20000` // ~20km radius
    }
...
export async function searchBusinesses(query, location, userCoords = null) {
  try {
