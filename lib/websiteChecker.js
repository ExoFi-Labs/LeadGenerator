      ...
      const placesWithDetails = await Promise.all(
        data.results.slice(0, 20).map(async (place) => {
          try {
            ...
            // Only request fields we need that aren't in text search: website, phone, opening_hours, etc.
            const detailsResponse = await fetch(
              `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place.place_id}&fields=formatted_phone_number,website,opening_hours,business_status,price_level,url&key=${process.env.GOOGLE_PLACES_API_KEY}`
            )
            const detailsData = await detailsResponse.json()
            
            // Use data from text search response (already have rating, types, name, address)
            // Only get additional fields from details if available
